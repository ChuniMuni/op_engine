...
//	if (conditions().IsSleeping())	return;
	...
	case kWPN_SCOPE_OPS:
		if (inventory().ActiveItem())
		{
			...
			if (weapon)
			{
				if (weapon->IsScopeAttached() && weapon->ScopeAttachable()) //если прицел стоит и можно сн€ть
				{
					mstate_wishful &= ~mcSprint;
	...
	case kWPN_GL_OPS:
		if (inventory().ActiveItem())
		{
			...
			if (weapon)
			{
				if (weapon->IsGrenadeLauncherAttached() && weapon->GrenadeLauncherAttachable()) //если ѕ√ стоит и можно сн€ть
				{
					mstate_wishful &= ~mcSprint;
					weapon->SwitchState(CWeapon::EWeaponStates::eDetachGL);
				}
				else if (weapon->GrenadeLauncherAttachable() && !weapon->IsGrenadeLauncherAttached())//если ѕ√ нет и можно одевать
				{
					mstate_wishful &= ~mcSprint;
...
//	if (conditions().IsSleeping())	return;
	...
	if (g_Alive())	
	{
		...
		case kDROP:		if(GAME_PHASE_INPROGRESS == Game().Phase()) g_PerformDrop(inventory().ActiveItem());				break;
