...
public:
	...
	virtual void Execute(LPCSTR /**args/**/) {
		...
		FATAL					("ENGINE CRASH.See log file for detail!");
		//int						*pointer = nullptr;
		//*pointer				= 0;
	}
};
...
struct CCC_JumpToLevel : public IConsole_Command {
	...
	virtual void Execute(LPCSTR args) {
		...
		if (0==xr_strlen(args) )
		{
			Log("- --- List levels in game graph start ---");
			for ( ; I != E; ++I)
				Log("-",(*I).second.name().c_str());
			Log("- --- List levels in game graph end ---");
			return;
		}
		string256		level;
		sscanf(args,"%s",level);
...
class CCC_LangReload : public IConsole_Command
{
public:
	CCC_LangReload(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = true; };

	void Execute(LPCSTR args) override
	{
#pragma todo("на данный момент нет механизма для частичной перезагрузки таблицы строк")
...
void CCC_RegisterCommands()
{
	...
	CMD1(CCC_LangReload, "lang_reload");
