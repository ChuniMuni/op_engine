...
void CScriptGameObject::IterateInventorySimple	(luabind::functor<void> functor)
{
	...
	TIItemContainer::iterator	E = inventory_owner->inventory().m_all.end();
	for ( ; B != E; ++B)
		functor				((*B)->object().lua_game_object());
}

void CScriptGameObject::IterateInventoryOnlyFunctor	(luabind::functor<void> functor)
{
	this->IterateInventory(functor,static_cast<luabind::object>(nullptr));
}
