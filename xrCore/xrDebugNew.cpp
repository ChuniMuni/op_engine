...
void xrDebug::backend	(const char *expression, const char *description, const char *argument0, const char *argument1, const char *file, int line, const char *function, bool &ignore_always)
{
	...
	buffer				+= sprintf(buffer,"%sPress OK to abort execution and see log for details%s",endline,endline);
...
\r\nMany thanks in advance and sorry for the inconvenience."
	...
	BT_SetFlags				(BTF_DETAILEDMODE | BTF_EDITMAIL | BTF_ATTACHREPORT |BTF_SHOWADVANCEDUI/**| BTF_LISTPROCESSES /**| BTF_SHOWADVANCEDUI /**| BTF_SCREENCAPTURE/**/);
	BT_SetDumpType			(
...
//		MiniDumpWithCodeSegs |
	...
	BT_SetSupportEMail		("winsor08@mail.ru");
...
#	endif // USE_OWN_MINI_DUMP
	...
	if (!error_after_dialog) {
		...
		//MessageBox			(nullptr,"Fatal error occured\n\nPress OK to abort program execution","Fatal error",MB_OK|MB_ICONERROR| MB_TOPMOST);
...
//		errno_t							err = 
	...
	static void std_out_of_memory_handler	()
	{
		...
		if ((std::get<1>(eco_strings) + std::get<2>(eco_strings)) > 500 * 1024 * 1024)
		{
			...
			handler_base("std: out of memory. В папке с логами создан str_dump.txt. Поделитесь ЭТИМ ФАЙЛОМ и ЛОГОМ ВЫЛЕТА с разработчиками мода.");
		}
		else
